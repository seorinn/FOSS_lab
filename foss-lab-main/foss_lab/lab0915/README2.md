...
```
```
> ln destination source: 하드링크 만듬 
> ln -s: 소프트링크(심볼링링크) 만듬
하드링크로 만들면 아이노드번(?)과 같음
파일이란게 만들어지면 리눅스 시스템에서는 모든것들이 번호로 관리가됨 파일이 관리가 되는건 아이노드번. 아이노드번이 어떻게 관리가 되냐면 똑같은 아이노드번이 생김 
하드링크를 만들어서 아이노드 똑같은 파일이 만들어졌으면 한쪽파일을 editing하면 양쪽 파일 다 editing됨 한쪽 파일 지우면 다른 한쪽은 남아있음. 어떤 아이노드를 가리키는 여러 파일이 있을 때 rm하면 레퍼런스 카운트만 하나씩 감소. 이게 1에서 0이 되는 순간 실제 파일을 지움
```
```
> pd: 현재 워킹 디렉토리
> cd ~: 내 홈 디렉토리
> cd ~oss000: oss000의 디렉토리로 감
```
